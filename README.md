...
### Server sign-in (both portals)

- **Finance app.** Signing in with role and PIN (`POST /api/auth/login`) returns a signed token
  (HMAC-SHA256). The app sends it on every call as `Authorization: Bearer …`. A device stays signed
  in for 30 days, renewed each time it is used. Every 12 hours the server quietly re-checks that the
  user still exists with the same role and PIN, so a PIN change or removal takes effect within 12 hours.
  An ended session shows a PIN prompt over the current page, and the interrupted save is retried once.
- **Keys need no setup.** The signing key and a separate read-only automation key are generated by
  the app on first use. They are stored in the private D1 table `app_secrets`, never returned by any
  route, and cached in memory. An optional `FINANCE_SESSION_SECRET` env var overrides the signing key.
- **Route groups.** Every `/api` route needs a Finance token unless it is on the public allowlist:
  logins, public KPSC pages, share links (`report-share` GET and POST), partnership pages, the Termii
  webhook, and cron/email-ingest routes, which have their own secrets. KPSC routes need a KPSC session.
  `settings`, `church-bank-ingest-log` and `bank-balance-snapshot` accept either.
- **Server-side roles.** Viewer is read-only. User management and sensitive settings (SMS/AI keys,
  permission maps) are admin-only. PIN guesses are limited to 5 per account and IP, then a
  15-minute lock.
- **Automations** (box scripts) read with `X-Automation-Key` (GET only). The key is in
  `app_secrets` under `automation_read_key`.
