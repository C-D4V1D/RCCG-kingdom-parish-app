...
const CORS_HEADERS = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type, X-KPSC-Session, Authorization, X-Automation-Key',
  'Access-Control-Expose-Headers': 'X-Finance-Token',
};
 ...
 * failure (401 / 403).
 */
// A request's KPSC session is looked up at most once: the auth middleware in
// onRequest resolves it first and the per-route requireKpscRole() checks below
// reuse the result, so locking the KPSC read routes adds no extra DB reads.
const _kpscAccountByRequest = new WeakMap();

async function resolveKpscSession(DB, request) {
  const memo = _kpscAccountByRequest.get(request);
  if (memo) return memo;
  ...
  _kpscAccountByRequest.set(request, account);
  return account;
}

async function requireKpscRole(DB, request, allowedRoles) {
  const account = await resolveKpscSession(DB, request);
...
// ── FINANCE APP AUTH ─────────────────────────────────────────────────
// The Finance app signs in with role + PIN at POST /api/auth/login, which now
// returns a signed token. Every other Finance request sends it as
// "Authorization: Bearer <token>". Checking it needs no database read: the
// signature, expiry and last re-check time are all inside the token.
//
// Token = "fin1.<base64url JSON payload>.<base64url HMAC-SHA256>".
// Payload: { v, uid, role, iat, exp, rv, pv } (seconds since epoch)
//   exp — 30 days, renewed at every 12-hour re-check (so a device stays signed
//         in while it's used at least once a month)
//   rv  — when the account was last re-checked against the users table
//   pv  — short keyed fingerprint of the stored PIN hash; a PIN change, role
//         change or user delete ends the session at the next re-check (≤12 h)
//
// The signing key is generated by the app itself on first use and kept in the
// private app_secrets D1 table (never returned by any route), then cached in
// isolate memory, so there is nothing to configure. If a FINANCE_SESSION_SECRET
// env var happens to exist it takes precedence, but it is not required.
// A second, separate key in the same table (automation_read_key) gives the
// parish's box automations read-only (GET) access — see checkAutomationKey.
const FIN_SESSION_TTL_S = 30 * 24 * 60 * 60;
const FIN_RECHECK_S     = 12 * 60 * 60;
const LOGIN_MAX_FAILS   = 5;
const LOGIN_LOCK_MS     = 15 * 60 * 1000;
const APP_SECRET_SIGNING    = 'finance_session_signing_key';
const APP_SECRET_AUTOMATION = 'automation_read_key';
const APP_SECRETS_DDL = `CREATE TABLE IF NOT EXISTS app_secrets (
      name        TEXT PRIMARY KEY,
      value       TEXT NOT NULL,
      created_at  TEXT DEFAULT (datetime('now'))
    )`;
const AUTH_THROTTLE_DDL = `CREATE TABLE IF NOT EXISTS auth_throttle (
      k             TEXT PRIMARY KEY,
...
const FIN_AUTH_MESSAGES = {
  auth_required: 'Please sign in again: the app was updated for security. Reload the page and enter your PIN.',
  reauth:        'Your sign-in has expired. Please enter your PIN again.',
};
function finAuthErr(code, status = 401, message) {
  return new Response(JSON.stringify({ error: message || FIN_AUTH_MESSAGES[code] || 'Sign-in required', code }),
...
// Isolate-memory cache: after the first read, normal requests never touch D1 for keys.
let _appSecrets = null;
let _appSecretsPromise = null;

async function readAppSecretRows(DB) {
  const { results } = await DB.prepare(`SELECT name, value FROM app_secrets WHERE name IN (?, ?)`)
    .bind(APP_SECRET_SIGNING, APP_SECRET_AUTOMATION).all();
  const rows = {};
  for (const r of (results || [])) if (r && r.name && r.value) rows[r.name] = String(r.value);
  return rows;
}

async function loadAppSecrets(DB) {
  let rows;
  try {
    rows = await readAppSecretRows(DB);
  } catch (e) {
    if (!/no such table/i.test(String(e?.message || e))) throw e;
    rows = {};
  }
  if (!rows[APP_SECRET_SIGNING] || !rows[APP_SECRET_AUTOMATION]) {
    // First use: create the table and the keys. INSERT OR IGNORE then re-read, so if
    // two isolates race, both end up using whichever row actually landed.
    await DB.prepare(APP_SECRETS_DDL).run();
    for (const name of [APP_SECRET_SIGNING, APP_SECRET_AUTOMATION]) {
      if (rows[name]) continue;
      await DB.prepare(`INSERT OR IGNORE INTO app_secrets (name, value) VALUES (?, ?)`)
        .bind(name, randomSecret(32)).run();
    }
    rows = await readAppSecretRows(DB);
  }
  if (!rows[APP_SECRET_SIGNING] || !rows[APP_SECRET_AUTOMATION]) throw new Error('app_secrets could not be read back');
  return { signing: rows[APP_SECRET_SIGNING], automation: rows[APP_SECRET_AUTOMATION] };
}

async function getAppSecrets(DB) {
  if (_appSecrets) return _appSecrets;
  if (!_appSecretsPromise) {
    _appSecretsPromise = loadAppSecrets(DB)
      .then(s => { _appSecrets = s; return s; })
      .catch(e => { _appSecretsPromise = null; throw e; });
  }
  return _appSecretsPromise;
}

async function getSigningSecret(DB, env) {
  const fromEnv = String(env?.FINANCE_SESSION_SECRET || '').trim();
  if (fromEnv) return fromEnv;
  return (await getAppSecrets(DB)).signing;
}

// An unauthenticated request still makes sure the keys exist (in the background,
// once per isolate), so the automation key is created by the very first request
// after deploy — not only after someone signs in.
function primeAppSecrets(context, DB) {
  if (_appSecrets || _appSecretsPromise) return;
  const p = getAppSecrets(DB).catch(e => console.warn('[auth] app secret init deferred:', e?.message || e));
  try { context?.waitUntil?.(p); } catch { /* not in a Worker (tests) */ }
}

const _hmacKeys = new Map();
async function hmacKeyFor(secret) {
  let k = _hmacKeys.get(secret);
  if (!k) {
    k = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret),
      { name: 'HMAC', hash: 'SHA-256' }, false, ['sign', 'verify']);
    if (_hmacKeys.size > 4) _hmacKeys.clear();
    _hmacKeys.set(secret, k);
  }
  return k;
}
async function hmacB64(secret, msg) {
  const sig = await crypto.subtle.sign('HMAC', await hmacKeyFor(secret), new TextEncoder().encode(msg));
  return b64urlEncode(new Uint8Array(sig));
}
async function pinFingerprint(secret, storedPin) {
  return (await hmacB64(secret, 'pv|' + String(storedPin || ''))).slice(0, 22);
}

async function signFinanceToken(secret, { uid, role, storedPin, iat, exp, rv }) {
  const now = nowSec();
  const payload = {
    v: 1, uid: String(uid), role: String(role),
    iat: iat ?? now, exp: exp ?? now + FIN_SESSION_TTL_S, rv: rv ?? now,
    pv: await pinFingerprint(secret, storedPin),
  };
  const body = b64urlEncode(new TextEncoder().encode(JSON.stringify(payload)));
  return `fin1.${body}.${await hmacB64(secret, 'fin1.' + body)}`;
}

async function verifyFinanceTokenSignature(secret, token) {
  const parts = String(token || '').split('.');
  if (parts.length !== 3 || parts[0] !== 'fin1' || !parts[1] || !parts[2]) return null;
  try {
    const ok = await crypto.subtle.verify('HMAC', await hmacKeyFor(secret), b64urlDecode(parts[2]),
      new TextEncoder().encode('fin1.' + parts[1]));
    if (!ok) return null;
    const p = JSON.parse(new TextDecoder().decode(b64urlDecode(parts[1])));
    if (!p || p.v !== 1 || !p.uid || !p.role || !Number.isFinite(p.exp) || !Number.isFinite(p.rv)) return null;
    return p;
  } catch {
    return null;
  }
...
// Returns { user, renewedToken? } or { response }.
async function checkFinanceToken(DB, env, token) {
  const secret = await getSigningSecret(DB, env);
  const p = await verifyFinanceTokenSignature(secret, token);
  const now = nowSec();
  if (!p || p.exp <= now) return { response: finAuthErr('reauth') };
  if (now - p.rv < FIN_RECHECK_S && p.rv <= now + 300) {
    return { user: { id: p.uid, role: p.role } };
  }
  // 12-hour re-check: one read confirming the user still exists with the same role and PIN.
  const row = await DB.prepare(`SELECT id, name, role, pin FROM users WHERE id=?`).bind(p.uid).first();
  if (!row || row.role !== p.role || (await pinFingerprint(secret, row.pin)) !== p.pv) {
    return { response: finAuthErr('reauth') };
  }
  const renewedToken = await signFinanceToken(secret, { uid: row.id, role: row.role, storedPin: row.pin, iat: p.iat });
  return { user: { id: row.id, role: row.role, name: row.name }, renewedToken };
}

async function issueFinanceToken(DB, env, row) {
  const secret = await getSigningSecret(DB, env);
  return signFinanceToken(secret, { uid: row.id, role: row.role, storedPin: row.pin });
}

async function checkAutomationKey(DB, key) {
  const { automation } = await getAppSecrets(DB);
  return timingSafeEqualStr(key, automation);
}

// ── Route groups ─────────────────────────────────────────────────────
// public  — no sign-in (logins, public pages, share links, webhooks/cron with their own secrets)
// kpsc    — KPSC portal routes: need a valid KPSC session (per-route role checks still apply)
// shared  — used by both portals: Finance token OR KPSC session
// finance — everything else, including any route added later: Finance token required
...
function classifyApiRoute(route, param, method) {
  ...
  if (route === 'auth' && ((POST && param === 'login') || (GET && param === 'options'))) return 'public';
  if (route === 'kpsc-login-options' && GET) return 'public';
  if ((route === 'kpsc-login' || route === 'kpsc-logout' || route === 'kpsc-change-pin') && POST) return 'public';
  if (route === 'kpsc-policy' && GET && (!param || param === 'summary')) return 'public';
  if ((route === 'kpsc-public-minutes' || route === 'kpsc-finance-report') && GET && param) return 'public';
  if (route === 'report-share' && ((POST && !param) || (GET && param))) return 'public';
  if ((route === 'partnership-public' || route === 'partnership-og-image' || route === 'partnership-favicon') && GET) return 'public';
  if ((route === 'partnership-pledge' || route === 'partnership-feedback') && POST) return 'public';
  if (route === 'termii-webhook' && POST) return 'public';   // HMAC-verified in handleTermiiWebhook
  if (route === 'internal') return 'public';                 // CRON_SECRET / EMAIL_INGEST_SECRET
  if (SHARED_ROUTES.has(route)) return 'shared';
  if (route.startsWith('kpsc-') || route.startsWith('voice-') || KPSC_NATIVE_ROUTES.has(route)) return 'kpsc';
  return 'finance';
}

// Coarse server-side role rules for Finance tokens (the app's finer
// per-button permissions are still applied in the browser as before).
function financeRoleDenied(user, route, param, method) {
  ...
  if (route === 'users' && user.role !== 'it_admin') {
    return finAuthErr('forbidden', 403, 'Only the IT administrator can manage users.');
  }
  return null;
}

// Returns { response } to stop the request, or { group, finance?, kpsc?, automation?, renewedToken? }.
async function authorizeApiRequest(context, DB, route, param, method) {
  const { request, env } = context;
  const group = classifyApiRoute(route, param, method);
  if (group === 'public') return { group };
  if (group === 'kpsc') {
    const account = await resolveKpscSession(DB, request);
    if (account instanceof Response) return { response: account };
    return { group, kpsc: account };
  }
  const authHeader = request.headers.get('Authorization') || '';
  const bearer = (/^Bearer\s+(\S+)/i.exec(authHeader) || [])[1] || '';
  if (bearer) {
    const r = await checkFinanceToken(DB, env, bearer);
    if (r.response) return r;
    const denied = financeRoleDenied(r.user, route, param, method);
    if (denied) return { response: denied };
    return { group, finance: r.user, renewedToken: r.renewedToken };
  }
  const automationKey = (request.headers.get('X-Automation-Key') || '').trim();
  if (automationKey) {
    if (method !== 'GET') return { response: finAuthErr('forbidden', 403, 'The automation key is read-only.') };
    if (!(await checkAutomationKey(DB, automationKey))) {
      return { response: finAuthErr('automation_key_invalid', 401, 'Automation key not recognised.') };
    }
    return { group, automation: true };
  }
  if (group === 'shared' && request.headers.get('X-KPSC-Session')) {
    const account = await resolveKpscSession(DB, request);
    if (account instanceof Response) return { response: account };
    return { group, kpsc: account };
  }
  primeAppSecrets(context, DB);
  return { response: finAuthErr('auth_required') };
}
...
// ── Login throttle ───────────────────────────────────────────────────
// 5 wrong PINs per account + IP → 15-minute lock. The attempt is counted
// atomically *before* the PIN is checked (so parallel guesses can't slip past),
// and a correct PIN deletes the row. Touched only by logins and PIN changes.
// Any DB problem here fails open: it must never stop a genuine sign-in.
let _throttleTableReady = false;
...
function lockedResponse(minutes) {
  ...
  return finAuthErr('locked', 429, `Too many incorrect PIN attempts. Please wait ${m} minute${m === 1 ? '' : 's'} and try again.`);
}
async function throttleAttempt(DB, key) {
  try {
    if (!_throttleTableReady) { await DB.prepare(AUTH_THROTTLE_DDL).run(); _throttleTableReady = true; }
    const now = Date.now();
    const row = await DB.prepare(`
      INSERT INTO auth_throttle (k, fails, locked_until, updated_at) VALUES (?, 1, 0, ?)
      ON CONFLICT(k) DO UPDATE SET
        fails        = CASE WHEN auth_throttle.locked_until > 0 AND auth_throttle.locked_until <= ? THEN 1 ELSE auth_throttle.fails + 1 END,
        locked_until = CASE WHEN auth_throttle.locked_until > 0 AND auth_throttle.locked_until <= ? THEN 0 ELSE auth_throttle.locked_until END,
        updated_at   = ?
      RETURNING fails, locked_until
    `).bind(key, now, now, now, now).first();
    if (!row) return { fails: 0, lockedMinutes: 0 };
    const lockedUntil = Number(row.locked_until) || 0;
    const fails = Number(row.fails) || 0;
    if (lockedUntil > now) return { fails, lockedMinutes: Math.ceil((lockedUntil - now) / 60000) };
    if (fails > LOGIN_MAX_FAILS) {
      await DB.prepare(`UPDATE auth_throttle SET locked_until=? WHERE k=?`).bind(now + LOGIN_LOCK_MS, key).run();
      return { fails, lockedMinutes: Math.ceil(LOGIN_LOCK_MS / 60000) };
    }
    return { fails, lockedMinutes: 0 };
  } catch (e) {
    console.warn('[auth] throttle check skipped:', e?.message || e);
    return { fails: 0, lockedMinutes: 0 };
  }
}
async function throttleFailed(DB, key, attempt) {
  if (!attempt || attempt.fails < LOGIN_MAX_FAILS) return;
  try {
    await DB.prepare(`UPDATE auth_throttle SET locked_until=? WHERE k=?`).bind(Date.now() + LOGIN_LOCK_MS, key).run();
  } catch (e) { console.warn('[auth] throttle lock skipped:', e?.message || e); }
}
async function throttleReset(DB, key) {
  try { await DB.prepare(`DELETE FROM auth_throttle WHERE k=?`).bind(key).run(); }
  catch (e) { console.warn('[auth] throttle reset skipped:', e?.message || e); }
}

// Test-only hooks (node --test). Not reachable from any route.
export const __authTesting = {
  setAppSecrets(s) { _appSecrets = s ? { ...s } : null; _appSecretsPromise = null; },
  resetThrottleTable() { _throttleTableReady = false; },
  signFinanceToken, verifyFinanceTokenSignature, classifyApiRoute, pinFingerprint,
  FIN_RECHECK_S, FIN_SESSION_TTL_S,
};
...
async function unlockAttendanceWeek(DB, weekEnd, body, request) {
  ...
  // Same 5-tries / 15-minute throttle as sign-in (shared per account + IP), so this
  // PIN check can't be used to guess the IT Administrator's PIN.
  const throttleKey = `fin:${userId}:${clientIp(request)}`;
  const attempt = await throttleAttempt(DB, throttleKey);
  if (attempt.lockedMinutes) return lockedResponse(attempt.lockedMinutes);
...
export async function onRequest(context) {
  ...
  // Sign-in check for every route (see FINANCE APP AUTH). Runs before the body
  // is read so multipart/audio routes still get their untouched request.
  let authz;
  try {
    authz = await authorizeApiRequest(context, DB, route, param, method);
  } catch (e) {
    console.error(`[API Auth] ${method} /api/${path}:`, e?.message || e);
...
async function routeApiRequest(context, { DB, url, method, path, parts, route, param, authz }) {
  const { request, env } = context;
  try {
    let body = null;
    ...
    if (route === 'auth') {
      if (method === 'POST' && param === 'login') return await loginUser(DB, body, request, env);
      if (method === 'GET' && param === 'options') return await getLoginOptions(DB);
    }
    if (route === 'kpsc-login-options' && method === 'GET') return await getKpscLoginOptions(DB);
    if (route === 'kpsc-login' && method === 'POST') return await kpscLoginUser(DB, body, request);
    if (route === 'kpsc-logout' && method === 'POST') return await kpscLogout(DB, body);
    if (route === 'kpsc-change-pin' && method === 'POST') return await changeKpscPin(DB, body, request);
    ...
    if (route === 'change-pin' && method === 'POST') {
      return await changeUserPin(DB, body, request, env, authz);
    }
    ...
    // ── /api/settings ──────────────────────────────────────────
    // NOTE: this endpoint is shared by both the KPSC portal (X-KPSC-Session)
    // and the Finance app (Bearer token), so the middleware accepts either.
    // getSettings() still never returns raw secrets (AI keys, Termii key,
    // default KPSC PIN), and saveSettingsAuthorized() keeps the sensitive
    // keys admin-only.
    if (route === 'settings') {
      if (method === 'GET'  && param === 'api-status')       return getApiStatus(env);
      if (method === 'GET'  && !param)                       return await getSettings(DB);
      if (method === 'POST' && !param)                       return await saveSettingsAuthorized(DB, body, authz);
...
async function handleInit(DB) {
  ...
  // Seed default users ONLY into an empty table (brand-new database). Once any
  // user exists, deleted default accounts are never re-created with their
  // well-known PINs.
  const userCount = await DB.prepare(`SELECT COUNT(*) AS n FROM users`).first();
  const seedUsers = Number(userCount?.n || 0) === 0;
  const defaultUsers = [
  ...
  for (const u of (seedUsers ? defaultUsers : [])) {
  ...
  const kpscCount = await DB.prepare(`SELECT COUNT(*) AS n FROM kpsc_accounts`).first();
  for (const acct of (Number(kpscCount?.n || 0) === 0 ? defaultKpscAccounts : [])) {
  ...
  return ok({
    success: true,
    message: seedUsers ? 'Database initialised. All tables created and default users seeded.' : 'Database initialised. All tables up to date.',
...
async function findLoginUser(DB, role, userId) {
  if (userId) {
    const row = await DB.prepare(`SELECT id,name,role,email,pin FROM users WHERE id=? AND role=?`).bind(userId, role).first();
    return { row: row || null };
  }
  const { results } = await DB.prepare(`SELECT id,name,role,email,pin FROM users WHERE role=? ORDER BY name`).bind(role).all();
  const users = results || [];
  if (users.length > 1) return { ambiguous: true };
  return { row: users[0] || null };
}

async function usersTableIsEmpty(DB) {
  try {
...
async function loginUser(DB, data, request, env) {
  ...
  let found;
  try {
    found = await findLoginUser(DB, role, userId);
  } catch (e) {
    if (!/no such table/i.test(String(e?.message || e))) throw e;
    found = { row: null };
  }
  // Brand-new database only: /api/init now needs a signed-in user, so the very
  // first sign-in creates the schema and default users. Never runs once any
  // user exists (handleInit itself also refuses to re-seed a non-empty table).
  if (!found.row && !found.ambiguous && await usersTableIsEmpty(DB)) {
    await handleInit(DB);
    found = await findLoginUser(DB, role, userId);
  }
  if (found.ambiguous) return err('Please select your name', 400);
  let row = found.row;

  const throttleKey = `fin:${row ? row.id : 'role:' + role}:${clientIp(request)}`;
  const attempt = await throttleAttempt(DB, throttleKey);
  if (attempt.lockedMinutes) return lockedResponse(attempt.lockedMinutes);

  if (!row) { await throttleFailed(DB, throttleKey, attempt); return err('Invalid credentials', 401); }
  const validPin = await verifyPin(row.pin, pin);
  if (!validPin) { await throttleFailed(DB, throttleKey, attempt); return err('Invalid credentials', 401); }
  if (!isHashedPin(row.pin)) {
    const upgraded = await hashPin(pin);
    await DB.prepare(`UPDATE users SET pin=? WHERE id=?`).bind(upgraded, row.id).run();
    row = { ...row, pin: upgraded };   // token's PIN fingerprint must match what's now stored
  }
  await throttleReset(DB, throttleKey);
  const token = await issueFinanceToken(DB, env, row);
  const mustChangePin = await getUserMustChangePin(DB, row.id);
  return ok(mustChangePin ? { ...publicUser(row), mustChangePin: true, token } : { ...publicUser(row), token });
}
...
async function changeUserPin(DB, data, request, env, authz) {
  const userId = String(data?.userId || '').trim();
  ...
  // A signed-in user may only change their own PIN (IT admin resets others via Users).
  if (authz?.finance && authz.finance.id !== userId) return err('You can only change your own PIN', 403);
  const row = await DB.prepare(`SELECT id, role, pin FROM users WHERE id=?`).bind(userId).first();
  if (!row) return err('User not found', 404);
  const throttleKey = `fin:${row.id}:${clientIp(request)}`;
  const attempt = await throttleAttempt(DB, throttleKey);
  if (attempt.lockedMinutes) return lockedResponse(attempt.lockedMinutes);
  const validCurrentPin = await verifyPin(row.pin, currentPin);
  if (!validCurrentPin) { await throttleFailed(DB, throttleKey, attempt); return err('Current PIN is incorrect', 401); }
  // The current PIN was right, so this attempt no longer counts towards a lock.
  await throttleReset(DB, throttleKey);
  if (newPin === currentPin) return err('Choose a new PIN that is different from your current PIN', 400);
  const newHash = await hashPin(newPin);
  await DB.prepare(`UPDATE users SET pin=? WHERE id=?`).bind(newHash, userId).run();
  await setUserMustChangePin(DB, userId, false);
  // This device keeps working with a fresh token; the user's other devices are
  // asked for the new PIN at their next 12-hour re-check.
  const token = await issueFinanceToken(DB, env, { id: row.id, role: row.role, pin: newHash });
  return ok({ success: true, id: userId, token });
}
...
async function kpscLoginUser(DB, data, request) {
  const accountId = String(data?.accountId || '').trim();
  const pin = String(data?.pin || '').trim();
  if (!accountId || !pin) return err('accountId and pin are required', 400);
  const throttleKey = `kpsc:${accountId}:${clientIp(request)}`;
  const attempt = await throttleAttempt(DB, throttleKey);
  if (attempt.lockedMinutes) return lockedResponse(attempt.lockedMinutes);
  ...
  if (!valid) { await throttleFailed(DB, throttleKey, attempt); return err('Invalid credentials', 401); }
  await throttleReset(DB, throttleKey);
  if (!isHashedPin(row.pin)) {
...
async function changeKpscPin(DB, data, request) {
  const accountId = String(data?.accountId || '').trim();
  const currentPin = String(data?.currentPin || '').trim();
  const newPin = String(data?.newPin || '').trim();
  ...
  const attempt = await throttleAttempt(DB, throttleKey);
  if (attempt.lockedMinutes) return lockedResponse(attempt.lockedMinutes);
  if (!(await verifyPin(row.pin, currentPin))) { await throttleFailed(DB, throttleKey, attempt); return err('Current PIN is incorrect', 401); }
  await throttleReset(DB, throttleKey);
...
async function getSettings(DB) {
  ...
  // Same for the Termii SMS key (SMS is sent server-side via getTermiiSettings)
  // and the seeded default KPSC PIN, which no browser code needs.
  out.kpsc_termii_api_key_set = !!String(out.kpsc_termii_api_key || '').trim();
  delete out.kpsc_termii_api_key;
  delete out.kpsc_default_pin;

  return ok(out);
}

// Keys only an admin may change: Finance IT admin, or a KPSC admin role.
...
function canonicalSettingValue(v) {
  const stored = (v !== null && typeof v === 'object') ? JSON.stringify(v) : String(v ?? '');
  try { return JSON.stringify(JSON.parse(stored)); } catch { return stored; }
}

async function saveSettingsAuthorized(DB, data, authz) {
  const privileged = authz?.finance ? authz.finance.role === 'it_admin'
    : authz?.kpsc ? KPSC_ADMIN_ROLES.includes(authz.kpsc.role) : false;
  const input = (data && typeof data === 'object') ? data : {};
  const toSave = {};
  const sensitive = [];
  for (const [key, value] of Object.entries(input)) {
    if (DERIVED_SETTING_KEYS.has(key)) continue;
    if (SENSITIVE_SETTING_KEYS.has(key) && !privileged) { sensitive.push([key, value]); continue; }
    toSave[key] = value;
  }
  // The Finance app saves settings as read-modify-write of the whole object, so
  // non-admin saves carry the permission maps unchanged. Unchanged values are
  // simply skipped; only an actual change is refused.
  if (sensitive.length) {
    const keys = sensitive.map(([k]) => k);
    ...
    for (const [key, value] of sensitive) {
      if (!(key in stored) || canonicalSettingValue(stored[key]) !== canonicalSettingValue(value)) {
        return err(`Only an administrator can change this setting (${key}).`, 403);
      }
    }
  }
  // A blank Termii key sent together with other SMS settings is the masked
  // "leave unchanged" field (older cached pages send it blank too) — keep the
  // stored key. Clearing is explicit: "Remove key" sends that key on its own.
  if ('kpsc_termii_api_key' in toSave && !String(toSave.kpsc_termii_api_key ?? '').trim() && Object.keys(toSave).length > 1) {
    delete toSave.kpsc_termii_api_key;
  }
  return saveSettings(DB, toSave);
}
...
// carries `by` (user name) and `role` (user role); the server writes
// "${by} (${role})", never "System", falling back to "Finance Portal" only
// when no name was sent at all (B3).
function budgetAuditActor(data) {
  const by = String(data?.by || '').trim();
  const role = String(data?.role || '').trim();
  if (!by) return 'Finance Portal';
  return role ? `${by} (${role})` : by;
}

function budgetActorName(data) {
  return String(data?.by || '').trim() || 'Finance Portal';
}

const BUDGET_MONTH_NAMES = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
function budgetFormatMonthLabel(key) {
  const match = String(key || '').match(/^(\d{4})-(\d{2})$/);
  if (!match) return '';
  const idx = parseInt(match[2], 10) - 1;
  return `${BUDGET_MONTH_NAMES[idx] || ''} ${match[1]}`.trim();
}

function budgetStatusPhrase(statusLabel, shortBy) {
  if (statusLabel === 'short') return `Short by ₦${Math.max(0, Math.round(Number(shortBy || 0))).toLocaleString('en-NG')}`;
  if (statusLabel === 'tight') return 'Tight';
  return 'Enough';
}
...
async function saveMonthlyBudget(DB, data) {
  const targetMonthKey = String(data?.monthKey || '').trim();
  if (!isValidMonthKey(targetMonthKey)) return err('monthKey must be YYYY-MM', 400);
  const budgets = await loadMonthlyBudgets(DB);
  const plan = budgets[targetMonthKey];
  if (!plan) return err('No budget plan found for that month', 404);
  if (plan.status === 'accepted') return err('This plan is accepted. Reopen it first.', 409);

  const rawLines = Array.isArray(data?.lines) ? data.lines : [];
  // C1: percentage remittance / fixed quotas are already deducted at the top and
  // can never be entered here again. RCCG Payments (rccg_proj) is exempt — it is
  // the right place for levies, programmes and upcoming demands.
  for (const line of rawLines) {
    const key = String(line?.key || '').trim();
    const label = String(line?.label || '').trim();
    if (key === BUDGET_LEGACY_REMITTANCE_CATEGORY) {
      return err('Zonal / Area Joint is a remittance line and cannot be entered here.', 400);
    }
    if (key !== BUDGET_RCCG_DEMANDS_KEY && /\b(remittance|quota)\b/i.test(label)) {
      return err('Remittance and quota lines are worked out separately and cannot be entered here.', 400);
    }
  }
...
async function acceptMonthlyBudget(DB, data) {
  ...
  const acceptedBy = budgetActorName(data);
  budgets[targetMonthKey] = {
    ...plan,
    status: 'accepted',
  ...
  await writeMonthlyBudgets(DB, budgets);
  await writeAuditLog(DB, 'budget_accepted', `Monthly budget accepted for ${targetMonthKey}`, budgetAuditActor(data));
  return ok({ plan: budgets[targetMonthKey] });
}

async function reopenMonthlyBudget(DB, data) {
  const targetMonthKey = String(data?.monthKey || '').trim();
  if (!isValidMonthKey(targetMonthKey)) return err('monthKey must be YYYY-MM', 400);
  const budgets = await loadMonthlyBudgets(DB);
  const plan = budgets[targetMonthKey];
  if (!plan) return err('No budget plan found for that month', 404);
  if (plan.status !== 'accepted') return err('Only an accepted plan can be reopened', 400);
  const reason = String(data?.reason || '').trim();
  if (!reason) return err('A reason is required to reopen an accepted plan', 400);
  ...
  const historyEntry = {
    ...
    role,
    reason,
    previouslyAcceptedBy: plan.acceptedBy || '',
    previouslyAcceptedAt: plan.acceptedAt || '',
  };
  budgets[targetMonthKey] = {
    ...plan,
    status: 'draft',
    acceptedAt: '',
    acceptedBy: '',
    history: [...(Array.isArray(plan.history) ? plan.history : []), historyEntry],
  };
  await writeMonthlyBudgets(DB, budgets);
  await writeAuditLog(DB, 'budget_reopened', `Monthly budget reopened for ${targetMonthKey}: ${reason}`, budgetAuditActor(data));
  return ok({ plan: budgets[targetMonthKey] });
}
